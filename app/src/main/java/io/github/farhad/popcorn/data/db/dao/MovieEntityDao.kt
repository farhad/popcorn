...
interface MovieEntityDao : BaseDao<MovieEntity> {
    ...
    @Query("SELECT * FROM Movies WHERE category = 'upcoming' AND updatedAt > :updatedAfter ORDER BY updatedAt ASC LIMIT :itemCount")
    fun getUpcomingMovies(updatedAfter: Instant, itemCount: Int): List<MovieEntity>

    @Query("SELECT * FROM Movies WHERE category = 'trending' AND updatedAt > :updatedAfter ORDER BY updatedAt ASC LIMIT :itemCount")
